'''
A CNN (Residual Network) example using TensorFlow library.
...
# Construct CNN 
h = Conv2D(x, [3,3,1,8], [1,2,2,1], 'SAME', 'conv1')  # shape: [Batch,14,14,4]
h = BatchNorm(h, is_train, decay=0.9, name='bn1')
h = tf.nn.relu(h)
h = tf.nn.max_pool(h, [1,3,3,1], [1,2,2,1], 'SAME')   # shape: [Batch,7,7,4]

# Residual block
s = h 
h = BatchNorm(h, is_train, decay=0.9, name='bn1a')
h = tf.nn.relu(h)
h = Conv2D(h, [3,3,8,8], [1,1,1,1], 'SAME', 'res1a')  # shape: [Batch,7,7,8]
h = BatchNorm(h, is_train, decay=0.9, name='bn1b')
h = tf.nn.relu(h)
h = Conv2D(h, [3,3,8,8], [1,1,1,1], 'SAME', 'res1b')  # shape: [Batch,7,7,8]
h += s

s = h 
h = BatchNorm(h, is_train, decay=0.9, name='bn2a')
h = tf.nn.relu(h)
h = Conv2D(h, [3,3,8,8], [1,1,1,1], 'SAME', 'res2a')  # shape: [Batch,7,7,8]
h = BatchNorm(h, is_train, decay=0.9, name='bn2b')
h = tf.nn.relu(h)
h = Conv2D(h, [3,3,8,8], [1,1,1,1], 'SAME', 'res2b')  # shape: [Batch,7,7,8]
h += s

h = BatchNorm(h, is_train, decay=0.9, name='bn2')
h = tf.nn.relu(h)
h = tf.reduce_mean(h, axis=[1,2]) # shape: [Batch,8]
logit = Dense(h, [8,10], 'fc1')
