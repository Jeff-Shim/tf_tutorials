'''
A Softmax regression learning algorithm example using TensorFlow library.
This example is using generated random distribution.
...
###### Generate 2D random data and Plot ########################################
...
for j in range(K):
    ...
    t = np.linspace(j*4,(j+0)*4,N) + np.random.randn(N)*0.2 # theta
...
y = tf.placeholder(tf.float32, [None, 3]) # 3 classes (R,G,B)
